 ...
 command = commandarr[i].command;
		...
		try {
         //Usage of execSync is avoided as it not supported in windows. Hence, made the exec command asynchronous.
			//var out = await(childProcess.spawnSync(command,{'stdio':['pipe','pipe','pipe']}));
			//var out = spawn(command, ['list', '-g', '-depth', '0'], { stdio: 'inherit' });
			//var out = require('child_process').execSync('sh', ['-c', command], {stdio: 'pipe'});
			//var out = require('child_process').execSync('sh', ['-c', command], {stdio: 'pipe'});
     ...
     out.stdout.on('data', (data) => {
	     ...
	     out.on('close', (code) => {
	      ...
	      var comm = { testnumber:i, catalog: catalog };
		return testoutput(comm, out.toString());
				})
...
console.log("in testoutput and returning true")
	return t;
}
